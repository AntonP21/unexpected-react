...
function installInto(expect) {
  var assertionGenerator = new _AssertionGenerator["default"]({
    ActualAdapter: _unexpectedHtmllikeJsxAdapter["default"],
  ...
  assertionGenerator.installInto(expect); // We can convert ReactElements to a renderer by rendering them - but we only do it for `with event`
  ...
  expect.addAssertion('<ReactElement> with event <string> <assertion?>', function (expect, subject, eventName) {
    ...
    return expect.apply(void 0, [renderer, 'with event', eventName].concat(assertion));
  });
  expect.addAssertion("<ReactElement> with event (".concat(_reactEventNames["default"].join('|'), ") <assertion?>"), function (expect, subject) {
  ...
  expect.addAssertion('<ReactElement> with event <string> <object> <assertion?>', function (expect, subject, eventName, eventArgs) {
    ...
    for (var _len3 = arguments.length, assertion = new Array(_len3 > 4 ? _len3 - 4 : 0), _key3 = 4; _key3 < _len3; _key3++) {
      assertion[_key3 - 4] = arguments[_key3];
    }

    return expect.apply(void 0, [renderer, 'with event', eventName, eventArgs].concat(assertion));
  });
  expect.addAssertion("<ReactElement> with event (".concat(_reactEventNames["default"].join('|'), ") <object> <assertion?>"), function (expect, subject, eventArgs) {
    return expect.apply(void 0, [subject, 'with event', expect.alternations[0], eventArgs].concat(_toConsumableArray(assertion)));
